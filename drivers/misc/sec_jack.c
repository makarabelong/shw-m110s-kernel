...
static irqreturn_t sec_jack_detect_irq_thread(int irq, void *dev_id)
{
	...
	unsigned int state = gpio_get_value(pdata->det_gpio) ^ (!pdata->det_active_high);

	//pr_info("%s A\n", __func__);
	/* prevent suspend to allow user space to respond to switch */
	wake_lock_timeout(&hi->det_wake_lock, WAKE_LOCK_TIME);

	// add for jack insert noise in call
	if(!prox_factorymode) {
		if(state) {
			proximity_jack_control(0);
			//printk(KERN_DEBUG "[JACK] %s :: %d", __func__, state);
			msleep(15);
		} else {
			if (isVoiceCall) {
				proximity_jack_control(1);
				//printk(KERN_DEBUG "[JACK] %s :: %d", __func__, state);
			}
		}
	}
