...
static unsigned int wm8994_read_hw(struct snd_soc_codec *codec, u16 reg)
{
	...
	if (ret != 2) {
		dev_err(codec->dev, "Failed to read %#x: %d\n", reg, ret);
		return 0;
	}
...
int wm8994_write(struct snd_soc_codec *codec, unsigned int reg, unsigned int value)
{
	...
	if (ret == 4) {
		...
		dev_err(codec->dev, "Failed to write %#x: %d\n", reg, ret);
		return ret;
	}
}
...
int wm8994_flag_control(int on, int iscapture)
{
...
#if 1 // 20110321, move MIXOUT PWR Control
int wm8994_ampcontrol_mixoutpwr(int on)
{
    struct snd_soc_codec *codec = wm8994_codec;
    struct wm8994_priv *wm8994 = codec->drvdata;
    int val;
...
#if 0 // 20110321, move MIXOUT PWR Control
		if ((wm8994->codec_state & CALL_ACTIVE) && (wm8994->cur_path == RCV)) {
			mdelay(50);
			val = wm8994_read(codec, 0x03);
			val &= ~(0x00C0);
...
static const char *playback_path[] = {"OFF", "RCV", "SPK", "HP", "3POLE", "BT", "SPK_HP", "EXTRA_DOCK_SPEAKER", };
static const char *voicecall_path[] = {"OFF", "RCV", "SPK", "HP", "3POLE", "BT", };
static const char *voipcall_path[] = {"OFF", "RCV", "SPK", "HP", "3POLE", "BT",  };
...
static const char *input_source_state[] = {"Default", "Voice Recognition", "Camcorder"};
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (wm8994->cur_path == (EXTRA_DOCK_SPEAKER)) {
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if((wm8994->cur_path != path_num) || !(wm8994->codec_state & CALL_ACTIVE))
	{
		...
		/* AIF1DAC1 Unmute, Mono Mix diable, Fast Ramp */
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	DEBUG_LOG("CHANGED INPUT_SOURCE STATE [%d] => [%d]", wm8994->input_source, control_flag);
...
static const struct snd_kcontrol_new wm8994_snd_controls[] = {
	...
	SOC_ENUM_EXT("Capture MIC Path", path_control_enum[3],
				wm8994_get_mic_path, wm8994_set_mic_path),

	SOC_ENUM_EXT("Input Source", path_control_enum[4],
...
#if 1 // test // DO NOT DELETE temporary
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (!pdata) {
		dev_err(&i2c->dev, "bad WM8994 platform data\n");
		goto err_bad_pdata;
	}
...
static int wm8994_suspend(struct platform_device *pdev, pm_message_t msg)
{
	...
	/* Check pin value (LDO_EN, XTAL_EN) */
	audio_power(0, 0);
